...
Provides a fully-typed, envelope-aware async client for the Life Graph v1 SaaS
API.  All ``/api/v1/`` endpoints automatically unwrap the
``{"data": ..., "meta": ...}`` response envelope.  Rate-limit headers are
tracked on every response.
...
class AsyncLifeGraph:
    """Asynchronous client for the Life Graph v1 API.
    ...
    async def memories(
        ...
        data = await self._get(self._api("/memories"), params=params)
    ...
    async def memory(self, id: str) -> Memory:
        ...
        """
        data = await self._get(self._api(f"/memories/{id}"))
    ...
    async def create_memory(
        ...
        data = await self._post(self._api("/memories"), json=body)
        return [Memory.from_dict(m) for m in data]

    async def update_memory(self, id: str, **fields: Any) -> Memory:
        ...
        Raises:
            NotFoundError: If no memory with the given ID exists.
        """
        data = await self._patch(self._api(f"/memories/{id}"), json=fields)
        return Memory.from_dict(data)
    ...
    async def delete_memory(self, id: str) -> None:
        ...
        Raises:
            NotFoundError: If no memory with the given ID exists.
        """
        await self._delete(self._api(f"/memories/{id}"))
    ...
    async def search(
        ...
        data = await self._post(self._api("/search"), json=body)
        return [Memory.from_dict(m) for m in data]

    async def ask(self, question: str, limit: int = 10) -> AskResult:
        ...
        data = await self._post(
            self._api("/ask"), json={"question": question, "limit": limit},
        )
        return AskResult.from_dict(data)

    async def recall(self, context: dict) -> RecallContext:
        ...
        """
        data = await self._post(self._api("/recall"), json={"context": context})
        return RecallContext.from_dict(data)

    async def recall_event(self, context: dict, event: str) -> list[Memory]:
    ...
    async def start_session(self, context: dict | None = None) -> Session:
        ...
        data = await self._post(self._api("/sessions"), json=body)
    ...
    async def end_session(self, session_id: str) -> Session:
        ...
        Returns:
            The finalized :class:`Session` with summary information.
        """
        data = await self._post(self._api(f"/sessions/{session_id}/end"))
        return Session.from_dict(data)
    ...
    async def session(self, session_id: str) -> Session:
        ...
        """
        data = await self._get(self._api(f"/sessions/{session_id}"))
        return Session.from_dict(data)
    ...
    async def sessions(self, limit: int = 10) -> list[Session]:
        ...
        """
        data = await self._get(self._api("/sessions"), params={"limit": limit})
        return [Session.from_dict(s) for s in data]

    async def heartbeat(self, session_id: str, context: dict) -> Session:
    ...
    async def create_intention(
        ...
        data = await self._post(self._api("/intentions"), json=body)
        return Intention.from_dict(data)

    async def intentions(self) -> list[Intention]:
        """List all active intentions.

        Returns:
            A list of :class:`Intention` objects.
        """
        data = await self._get(self._api("/intentions"))
        return [Intention.from_dict(i) for i in data]

    async def triggered_intentions(self, context: dict) -> list[Intention]:
        ...
        """
        data = await self._post(
            self._api("/intentions/triggered"), json={"context": context},
        )
        return [Intention.from_dict(i) for i in data]

    async def complete_intention(self, id: str) -> Intention:
        ...
        """
        data = await self._post(self._api(f"/intentions/{id}/complete"))
        return Intention.from_dict(data)

    async def dismiss_intention(self, id: str) -> Intention:
        ...
        Raises:
            NotFoundError: If no intention with the given ID exists.
        """
        data = await self._post(self._api(f"/intentions/{id}/dismiss"))
        return Intention.from_dict(data)
    ...
    async def timeline(self, domain: str | None = None) -> list[TimelineChapter]:
        ...
        data = await self._get(self._api("/identity/timeline"), params=params)
        return [TimelineChapter.from_dict(c) for c in data]

    async def beliefs(self, domain: str | None = None) -> list[Memory]:
        ...
        if domain is not None:
            params["domain"] = domain
        data = await self._get(self._api("/identity/beliefs"), params=params)
        return [Memory.from_dict(m) for m in data]

    async def stale_beliefs(self, days: int = 90) -> list[StaleBelief]:
        ...
        data = await self._get(
            self._api("/identity/beliefs/stale"), params={"days": days},
        )
        return [StaleBelief.from_dict(b) for b in data]

    async def challenge_belief(self, memory_id: str) -> dict:
        ...
        """
        return await self._post(self._api(f"/identity/beliefs/{memory_id}/challenge"))

    # =====================================================================
    # Agent
    # =====================================================================

    async def build_context(self, task: str, project: str | None = None) -> dict:
        ...
        return await self._post(self._api("/agent/context"), json=body)

    async def learn(self, conversation: str, context: dict | None = None) -> list[Memory]:
        ...
        if context is not None:
            body["context"] = context
        data = await self._post(self._api("/agent/learn"), json=body)
        return [Memory.from_dict(m) for m in data]
    ...
    async def stats(self) -> Stats:
        ...
        """
        data = await self._get(self._api("/admin/stats"))
        return Stats.from_dict(data)

    async def gaps(self) -> list[KnowledgeGap]:
        """List detected knowledge gaps.

        Returns:
            A list of :class:`KnowledgeGap` objects.
        """
        data = await self._get(self._api("/admin/gaps"))
    ...
    async def ingest(
        ...
        if source is not None:
            body["source"] = source
        data = await self._post(self._api("/admin/ingest"), json=body)
        return [Memory.from_dict(m) for m in data]
    ...
    async def export_memories(self) -> dict:
        ...
        """
        return await self._get(self._api("/admin/export"))
    ...
    async def consolidate(self) -> dict:
        ...
        """
        return await self._post(self._api("/admin/consolidate"))

    async def jobs(self, limit: int = 20) -> list[JobRun]:
        ...
        """
        data = await self._get(self._api("/admin/jobs"), params={"limit": limit})
        return [JobRun.from_dict(j) for j in data]

    async def enqueue_consolidation(self, tenant_id: str | None = None) -> dict:
        ...
        return await self._post(self._api("/admin/consolidate/enqueue"), json=body)
    ...
    async def entities(self, label: str | None = None) -> list[GraphEntity]:
        ...
        if label is not None:
            params["label"] = label
        data = await self._get(self._api("/graph/entities"), params=params)
    ...
    async def entity(self, name: str) -> dict:
        ...
        """
        return await self._get(self._api(f"/graph/entity/{name}"))
    ...
    async def graph_query(
        ...
        return await self._post(self._api("/graph/query"), json=body)
    ...
    async def graph_search(
        ...
        if min_importance is not None:
            body["min_importance"] = min_importance
        return await self._post(self._api("/graph/search"), json=body)
