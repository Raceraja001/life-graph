...
>
> **Calibration loop closed (September 2026).** Until then nothing wrote `calibration_snapshots` and nothing created predictions except a raw API call. Now `services/calibration.py` computes live reports and writes a daily snapshot (nightly `run_nightly_calibration`, and after each manual resolve). `services/prediction_detection.py` turns explicit claims in the user's own captures ("I'm 80% sure X by Friday") into *suggested* predictions (`outcome="suggested"`) to accept or dismiss. The dashboard Calibration page logs, resolves and plots them. Expired predictions are asked about once, with a 7-day expiry. **Still open:** automatic resolvers (kernel task / git / watcher criteria), duration-based `estimate_multiplier` (the binary-outcome value is kept out of agent context and the advisor), and surfacing new bias findings in the daily brief.
