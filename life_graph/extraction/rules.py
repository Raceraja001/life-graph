...
_PREFERENCE_PATTERNS: list[tuple[re.Pattern[str], str, float]] = [
    ...
    (
        re.compile(
            r"\bi\s+(?:mostly|usually|typically)\s+use\s+(.+?)(?:[.,;!?\n]|$)", re.IGNORECASE
        ),
        "preference",
        0.85,
    ),
...
_ANTI_PREFERENCE_PATTERNS: list[tuple[re.Pattern[str], str, float]] = [
    ...
    (
        re.compile(
            r"\bi\s+(?:can'?t\s+stand|dislike|despise)\s+(.+?)(?:[.,;!?\n]|$)", re.IGNORECASE
        ),
        "anti_preference",
        0.85,
    ),
...
_DECISION_PATTERNS: list[tuple[re.Pattern[str], str, float]] = [
    ...
    (
        re.compile(
            r"\bi\s+(?:ended\s+up|settled\s+on)\s+(?:using\s+|with\s+)?(.+?)(?:[.,;!?\n]|$)",
            re.IGNORECASE,
        ),
        "decision",
        0.85,
...
_INTENTION_PATTERNS: list[tuple[re.Pattern[str], str, float]] = [
    ...
    (
        re.compile(
            r"\bi'?ll\s+(?:get\s+to|work\s+on|look\s+into)\s+(.+?)(?:[.,;!?\n]|$)", re.IGNORECASE
        ),
        "intention",
        0.75,
    ),
