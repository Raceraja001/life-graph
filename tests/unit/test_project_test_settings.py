...
def test_always_on_checks_are_appended_even_with_no_required_checks():
    """Not project-configurable — every dispatch gets these, opt-out is not offered."""
    assert _with_required_checks(["build_ok_diff"], {}) == [
        "build_ok_diff",
        "no_secrets_in_diff",
        "no_vulnerable_deps_in_diff",
    ]
    assert _with_required_checks(["no_secrets_in_diff", "no_vulnerable_deps_in_diff"], {}) == [
        "no_secrets_in_diff",
        "no_vulnerable_deps_in_diff",
    ]
