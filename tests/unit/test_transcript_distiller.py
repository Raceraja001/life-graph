...
def _distiller(sess, stored_returns):
    session = MagicMock()
    ...
    manager = MagicMock()
    manager.store_facts = AsyncMock(return_value=stored_returns)
    minio = MagicMock()
    minio.download = MagicMock(return_value=RAW.encode("utf-8"))
    minio.upload = MagicMock(return_value="http://x")
    store = MagicMock()
    store.update = AsyncMock()
    d = TranscriptDistiller(factory, manager, minio, store, PARSERS, MagicMock())
...
async def test_distill_extracts_new_turns_and_archives(monkeypatch):
    ...
    async def fake_extract(turns, *, resilient_llm):
        # The window handed to the extractor must already be redacted.
        joined = " ".join(t["text"] for t in turns)
        assert "sk-abcDEF1234567890abcdef" not in joined
        return [
            ExtractedFact(
                content="User prefers OpenRouter free models.",
