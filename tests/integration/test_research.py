...
class TestTriggerResearch:
    ...
    async def test_trigger_research_returns_202(
        ...
        assert response.status_code in (200, 202, 500), (
            f"Expected 200/202 or 500, got {response.status_code}: {response.text}"
        )
    ...
    async def test_trigger_with_preference_id(
        ...
        assert response.status_code != 422, f"Valid UUID should not return 422: {response.text}"
        assert response.status_code in (200, 202, 500)
...
class TestTenantIsolation:
    ...
    @pytest.mark.asyncio
    @skip_on_db_error
    async def test_missing_tenant_header_returns_error(self, monkeypatch):
        """Requests without X-Tenant-ID are rejected outside development.

        TenantMiddleware substitutes DEV_DEFAULT_TENANT when
        settings.is_development, so in the dev environment this request
        legitimately succeeds and the assertion below could never hold.
        Pin a non-development environment so the enforcement path is the one
        under test.
        """
        from life_graph.config import settings

        monkeypatch.setattr(settings, "environment", "production")
        # Auth runs before Tenant in the middleware stack, so without a key
        # this would 401 before the tenant check is ever reached. Supply one
        # so the only gate left to fail is the missing tenant header.
        ...
        async with AsyncClient(
            transport=transport,
            base_url="http://test",
            headers={"X-API-Key": "test-api-key"},  # no X-Tenant-ID
        ) as no_tenant_client:
